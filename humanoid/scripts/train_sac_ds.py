#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2026/5/30 15:48
# @Author  : cychn
# @File    : train_sac_ds.py
# @Software: PyCharm
...
# -*- coding: utf-8 -*-
"""
Train Unitree G1 walking with SAC (Soft Actor-Critic) using Stable-Baselines3.

Usage:
    python train_sac.py --num-envs 4 --total-timesteps 2_000_000
"""
...
try:
    ...
    from stable_baselines3.common.noise import NormalActionNoise  # SAC 不需要，但可选
...
DEFAULT_OUTPUT = ROOT_DIR / "outputs" / "sac_train"

# ---------- 环境配置 (沿用你定义好的 G1WalkingConfig) ----------
@dataclass
...
# ---------- 环境类 (完全复用你的 UnitreeG1WalkingEnv) ----------
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--total-timesteps", type=int, default=2_000_000)   # SAC 通常需要更少步数
    parser.add_argument("--num-envs", type=int, default=4)                  # SAC 可以用较少的环境
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"[sac_train] device={device}")
    print(f"[sac_train] xml={cfg.xml_path}")

    # 创建向量化环境 (SAC 也可以用 VecNormalize，但通常不需要归一化奖励)
    vec_env = build_vec_env(cfg, args.num_envs, monitor_dir)
    # SAC 对奖励尺度较敏感，建议启用 reward normalization
    ...
    # ---------- 关键：创建 SAC 模型 ----------
    # 注意：SAC 的 policy 必须使用 'MlpPolicy'，且需要指定网络架构
    model = SAC(
        "MlpPolicy",
        vec_env,
        device=device,
        ...
        learning_rate=3e-4,             # 学习率（与 PPO 类似）
        batch_size=256,                 # 每次从 buffer 中采样 256 条
        tau=0.005,                      # 软更新参数
        gamma=0.99,                     # 折扣因子
        train_freq=1,                   # 每步都训练
        gradient_steps=1,               # 每步梯度更新次数
        ent_coef="auto",                # 自动调整熵系数
        target_entropy="auto",          # 目标熵自动计算
        use_sde=False,                  # 不使用状态相关探索噪声（可选）
        sde_sample_freq=-1,
        # ---------- 网络结构 ----------
        policy_kwargs=dict(
            net_arch=[256, 256, 128],   # 与 PPO 一致
    ...
    # 如果要从 checkpoint 恢复
    if args.resume:
        print(f"[sac_train] resume={args.resume}")
        model = SAC.load(str(args.resume), env=vec_env, device=device)
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        name_prefix="g1_walk_sac",
        save_replay_buffer=True,        # SAC 需要保存 replay buffer 才能完整恢复
    ...
    model.learn(
        ...
        progress_bar=True,
        tb_log_name="sac_g1_walk",
    )

    # 保存最终模型
    final_model = model_dir / "g1_walk_sac_final.zip"
    final_norm = model_dir / "g1_walk_sac_final.vecnormalize.pkl"
    model.save(str(final_model))
    vec_env.save(str(final_norm))
    print(f"[sac_train] saved model: {final_model}")
    print(f"[sac_train] saved vecnormalize: {final_norm}")
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    model = SAC.load(str(args.resume), env=vec_env, device="cuda" if torch.cuda.is_available() else "cpu")
