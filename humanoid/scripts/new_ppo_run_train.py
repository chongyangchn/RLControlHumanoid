...
# -*- coding: utf-8 -*-
"""
Train Unitree G1 running with a mature PPO implementation.
...
Example:
    python humanoid/scripts/new_ppo_run_train.py --total-timesteps 8000000 --num-envs 8

Tip:
    Running is much easier if you warm-start from the walking policy:
    python humanoid/scripts/new_ppo_run_train.py --resume outputs/new_ppo_train/models/g1_walk_ppo_final.zip
"""
...
DEFAULT_OUTPUT = ROOT_DIR / "outputs" / "new_ppo_run_train"


@dataclass
class G1RunningConfig:
    ...
    command_x_range: Tuple[float, float] = (0.65, 1.35)
    command_yaw_range: Tuple[float, float] = (-0.15, 0.15)
    randomize_commands: bool = True
    healthy_height_range: Tuple[float, float] = (0.55, 1.10)
    terminate_upright_threshold: float = 0.50
    reset_noise_scale: float = 0.015
    gait_period: float = 0.48
...
class UnitreeG1RunningEnv(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 50}

    def __init__(self, cfg: Optional[G1RunningConfig] = None, render_mode: Optional[str] = None):
        super().__init__()
        self.cfg = cfg or G1RunningConfig()
    ...
    def step(self, action: np.ndarray):
        ...
        target[12:15] = self.default_ctrl[12:15] + 0.45 * self.cfg.action_scale * action[12:15]
        target[15:] = self.default_ctrl[15:] + 0.30 * self.cfg.action_scale * action[15:]
        ...
        info = {
            ...
            "reward_terms": terms,
            "is_success": bool(terms["base_vx"] > 0.75 and not terminated),
        }
    ...
    def _compute_reward(self, action: np.ndarray) -> Tuple[float, Dict[str, float]]:
        ...
        vel_track = exp_reward(local_lin_vel[:2] - np.array([self.command[0], 0.0]), 0.45)
        yaw_track = exp_reward(float(local_ang_vel[2] - self.command[2]), 0.35)
        height_reward = exp_reward(height - self.cfg.target_height, 0.14)
        ...
        flight_bonus = 1.0 if contacts.sum() == 0.0 else 0.0
        ...
        swing_clearance = 0.0
        if contacts[0] < 0.5:
            swing_clearance += exp_reward(left_z - 0.12, 0.10)
        if contacts[1] < 0.5:
            swing_clearance += exp_reward(right_z - 0.12, 0.10)
        ...
        forward_bonus = max(0.0, min(base_vx, self.command[0] + 0.35))
        ...
        reward = (
            ...
            + 0.45 * gait_contact
            + 0.25 * flight_bonus
            + 0.25 * swing_clearance
            + 0.15 * forward_bonus
            + 0.05
            - 0.18 * action_rate
            - 0.035 * action_mag
            - 0.28 * leg_error
            - 0.85 * waist_arm_error
            - 0.006 * joint_vel
            - 0.015 * ctrl_change
            - 0.35 * float(abs(local_lin_vel[1]))
            - 0.16 * float(abs(local_ang_vel[0]) + abs(local_ang_vel[1]))
            - 0.15 * float(stance_slide)
        )
        ...
        terms = {
            ...
            "flight_bonus": float(flight_bonus),
...
def make_env(cfg: G1RunningConfig, rank: int, monitor_dir: Path):
    def _init():
        env = UnitreeG1RunningEnv(cfg)
        return Monitor(env, filename=str(monitor_dir / f"env_{rank}.csv"))

    return _init


def build_vec_env(cfg: G1RunningConfig, num_envs: int, monitor_dir: Path):
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"[new_ppo_run_train] device={device}, cuda_available={torch.cuda.is_available()}")
    print(f"[new_ppo_run_train] xml={cfg.xml_path}")
    ...
    if args.resume:
        print(f"[new_ppo_run_train] resume={args.resume}")
    ...
    model.learn(
        ...
        progress_bar=True,
        tb_log_name="ppo_g1_run",
    )

    final_model = model_dir / "g1_run_ppo_final.zip"
    final_norm = model_dir / "g1_run_ppo_final.vecnormalize.pkl"
    model.save(str(final_model))
    vec_env.save(str(final_norm))
    print(f"[new_ppo_run_train] saved model: {final_model}")
    print(f"[new_ppo_run_train] saved vecnormalize: {final_norm}")
    vec_env.close()
    eval_env.close()
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    cfg = G1RunningConfig(xml_path=args.xml, command_x=args.command_x, randomize_commands=False)
    env = UnitreeG1RunningEnv(cfg, render_mode="human" if args.render else None)
